...
> **Note:** `Total_Invoiced` / `Total_Paid` / `Total_Expenses` are Currency fields on Projects updated by the Scheduled Workflow (not formula fields). These fields auto-calculate from report summaries. Actual P&L is computed via reports and dashboard widgets.
