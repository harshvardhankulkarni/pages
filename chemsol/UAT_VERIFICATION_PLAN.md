...
| **C30** | F11 correction found during verification completion: AUTOMATION_ALIGNMENT_PLAN claimed "forms.html + UAT plan use 4 MR states", but re-verification shows forms.html line 735, automation.html blueprint STATE 2, this UAT plan Step 5, BRD, data-model, flow, reports and implementation-plan ALL use the **5-state set** (Draft → Pending Production Verification → Production Verified → Costing Approved → Released). Only the AGENTS.md shorthand had 4 | ✅ **APPLIED** — 5-state set confirmed canonical (forms.html source of truth); AGENTS.md root + chemsol and REPORT_IMPLEMENTATION_PLAN updated to 5 states; alignment plan F11/A-17 marked RESOLVED; flow_sim.py now models all 5 transitions + SLA per stage |
| **C31** | Cross-validation tolerance defined at aggregate level (Σ Assigned vs Σ BOM expected) in the alignment plan, but UAT Step 5 mutates **one** Assigned Qty line by 6%/11% — a single-line error drowns in the aggregate (275→291.5 vs Σ400 → 3.4% diff = flag threshold 5% missed on 6% error). Rule refined: variance computed **per RM line** (`|Assigned − BOM expected| ÷ expected`), >5% flag, >10% block — matches UAT Step 5 exactly | ✅ **APPLIED** — crossValidate.deluge + flow_sim.py now per-item; mutation tests pass: +6% on RM-001 → flag only, +11% → block, revert → clean (sim 68/68 green) |
| **C32** | Post-audit product-bug fixes (crossValidate/gate, getAvailableStock, mrSlaSchedules, poGstSplit, mrDerive, checkAllocationAlert, fgConsumption + NEW costingSlaEscalate.deluge for A-08 24h escalation): SO_Master has NO Project_ID (would throw at runtime — now resolved via Costing_Sheet_Ref→SO_Reference); available-stock lookups missed "Pending Production Verification" (double-allocation hole); SLA reminder re-fired every 5 min (now guarded by SLA_Reminder_Sent); PO footer GST halved a single % (now per-line CGST/SGST/IGST sums); Allocation_Ratio hardcoded /400 (now per-project total); 80% alert never set Alert_Triggered (dashboard flag); FG consumption never created Project_FG_Consumption (G9/R6). automation.html cards synced to all of the above (2A, STATE 4/C15, 4B footer, 80% alert, SLA cadence, F6 filter note, numbering P1 pattern) | ✅ **APPLIED** — all 8 deluge files fixed + sim extended to 72 asserts (supplier seed, partial GRN rollback, 3b auto-derive, damaged-MRT rollback, R3 variance ₹37,270, R6 SCE log count) → **72/72 green** |
| **C33** | C6 sweep had missed residues: BRD :283/:330/:878, DELIVERY_PLAYBOOK :23/:70/:129/:155, alignment :19 + F3 row (:105), feasibility :88/:91/:92/:132 (MIS/Costing rows + 4-stage), index :498/:506, IMPLEMENTATION_PLAN :219/:880/:976, implementation-plan.html :394/:1158, zoho-projects-import :21, REPORT_IMPLEMENTATION_PLAN G5 (Not Started/Partial/Complete + Total Amount formula), G7 (FG Category), R4/R5/R6 group-by fixes (PO line Item Code, RM Consumption BMR ref, FG Position source), R7 widget notes | ✅ **APPLIED** — all swept to "SO acceptance → Costing Sheet; Project on Costing Approved" + 5-state MR set; F3 marked RESOLVED |
...
**Doc-level verification (loop passes 1–34):** every step's fields exist in forms.html, every expected automation has a hook in automation.html, every verify report is spec'd in reports.html R1–R7, every number traces end-to-end (SO ₹175,000 → Costing ₹146,000 → MR ₹144,000 → P&L +₹31,000; RM 275/125 kg; FG 148/300 kg; stock 10/285 kg) → all 10 steps + report sweep **doc-verified ✓** (see REPORT_IMPLEMENTATION_PLAN Phase 8; change log C1–C33 all applied; flow simulator 72/72 green).
