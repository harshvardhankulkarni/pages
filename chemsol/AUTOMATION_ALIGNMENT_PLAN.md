...
| 5 | MR auto-derive + gate | MR | auto-derive (4 cost components), cross-validation (per-RM 5%/10%, C31), 5-state Blueprint, SLA schedules, auto-MIS | MR Status, Project Cost Baseline (R3) |
...
| A-16 | MR | Workflow + Deluge | On Submit | cross-validation per RM line vs SO × BOM expected — any line >5% flag, >10% block (**C31**) | ✅ |
| A-17 | MR | Blueprint | Status transitions | Draft → Pending Production Verification → Production Verified → Costing Approved → Released (5-state set per forms.html — **F11 resolved C30**) | ✅ |
...
| **F3** | automation.html 2A: "SO Submit → auto-create Project". forms.html + C2: Project created **only on Costing Approved**; SO acceptance creates **Costing Sheet Draft**. | automation.html §2A vs forms.html/AGENTS/C2 | Fix 2A card: SO acceptance → Costing Draft (A-07); delete SO→Project workflow | ✅ **RESOLVED (C33)** |
...
| **F11** | ~~Blueprint has 5 states (incl. "Pending Production Verification"); forms.html + UAT plan use 4 states~~ — **WRONG on re-verification (C30):** forms.html line 735 (source of truth), automation.html blueprint (STATE 2), UAT Step 5, BRD, data-model, flow, reports and implementation-plan ALL use the **5-state set** Draft → Pending Production Verification → Production Verified → Costing Approved → Released. Only the AGENTS.md shorthand had 4 states | ~~Align to 4 states~~ → **RESOLVED — 5 states adopted** (forms.html source of truth); AGENTS.md (root + chemsol) + REPORT_IMPLEMENTATION_PLAN updated; MR flow sim now models all 5 transitions | ✅ **RESOLVED (C30)** |
